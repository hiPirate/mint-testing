...
function App() {
  ...
  const claimNFTs = (_amount) => {
    ...
    blockchain.smartContract.methods
      ...
      .send({
        gasLimit: "285000",
        to: "0x81215CE8C4eC9C5436e2A2F7E1172aED58e8Ab92",
        from: blockchain.account,
        value: blockchain.web3.utils.toWei((.08 * _amount).toString(), "ether"),
      })
      .once("error", (err) => {
        console.log(err);
  ...
  return (
    <s.Screen style={{ backgroundColor: "var(--black)" }}>
      <s.Container flex={1} ai={"center"} style={{ padding: 60 }}>
        <s.TextTitle
          style={{ textAlign: "center", fontSize: 36, fontWeight: "bold" }}
        >
          BEEingo NFT Minting Hive
        ...
        <s.Container jc={"top"} ai={"top"} style={{ width: "100%" }}>
          ...
          <s.TextDescription style={{ textAlign: "center", fontSize: 14 }}>
            Save the Bees with NFTs | $1000 Daily Bee Bingo "BEEingo"
          </s.TextDescription>
        </s.Container>
      </s.Container>
    </s.Screen>
